...
public class Match extends BaseMatch<Match> {
	...
	public Integer getStopSeq(int martchId){
		String sql = "select (select count(*) from t_match_user u where u.MATCH_ID = ? and u.SEQ is not null) - " +
		...
		int losePlayer = Db.queryLong(sql, martchId, martchId, martchId).intValue();
